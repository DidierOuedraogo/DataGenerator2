...
# Logo de l'industrie minière (généré par code)
def generate_mining_logo():
    ...
    # Dessiner un casque de mineur stylisé
    theta = np.linspace(0, 2*np.pi, 100)
    r = 0.8
    x = r * np.cos(theta)
    y = r * np.sin(theta) + 0.1
    ax.plot(x, y, color='#FFD700', linewidth=3)
    
    # Dessiner la visière du casque
    ax.plot([-0.5, 0.5], [0.1, 0.1], color='#FFD700', linewidth=3)
    
    # Dessiner une pioche et une pelle croisées
    ax.plot([-0.7, 0.7], [-0.5, -0.9], color='#A0A0A0', linewidth=2)
    ax.plot([-0.7, 0.7], [-0.9, -0.5], color='#A0A0A0', linewidth=2)
    
    # Ajouter un détail de lampe
    ax.add_patch(plt.Circle((0, 0.5), 0.15, color='#FFF9C4'))
...
with col1:
    st.image(generate_mining_logo(), width=100)
with col2:
